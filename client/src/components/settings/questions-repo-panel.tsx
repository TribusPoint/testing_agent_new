...
export default function QuestionsRepoPanel() {
    ...
    <div className="flex flex-col gap-4 max-w-5xl">
      <div className="flex flex-wrap items-end gap-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-3">
        ...
        <div className="flex flex-col gap-1 min-w-[8rem]">
          ...
          <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} disabled={!filterDomain} className={`${SELECT_CLS} disabled:opacity-50`}>
        ...
        <button type="button" onClick={() => { setFilterDomain(""); setFilterCategory(""); setSearchText(""); }} className="text-xs text-gray-500 hover:text-gray-800 dark:hover:text-gray-300 px-2 py-1.5">
          Clear
        </button>
        <button type="button" onClick={() => setShowAdd((v) => !v)} className="text-xs bg-indigo-600 text-white px-3 py-1.5 rounded-lg hover:bg-indigo-700 ml-auto">
      ...
      {showAdd && (
        <div className="bg-white dark:bg-gray-900 border border-indigo-200 dark:border-indigo-900/50 rounded-lg p-4">
          ...
          <div className="flex flex-col gap-2 max-w-2xl">
            <textarea rows={2} placeholder="Question text *" value={addForm.question} onChange={(e) => setAddForm((f) => ({ ...f, question: e.target.value }))} className={INPUT_CLS + " resize-none"} />
            <textarea rows={2} placeholder="Expected answer (optional)" value={addForm.expected_answer} onChange={(e) => setAddForm((f) => ({ ...f, expected_answer: e.target.value }))} className={INPUT_CLS + " resize-none"} />
            <div className="grid grid-cols-3 gap-2">
              <input placeholder="Domain" value={addForm.domain} onChange={(e) => setAddForm((f) => ({ ...f, domain: e.target.value }))} className={INPUT_CLS} />
              <input placeholder="Category" value={addForm.category} onChange={(e) => setAddForm((f) => ({ ...f, category: e.target.value }))} className={INPUT_CLS} />
              <input placeholder="Tags" value={addForm.tags} onChange={(e) => setAddForm((f) => ({ ...f, tags: e.target.value }))} className={INPUT_CLS} />
            </div>
            <div className="flex gap-2 pt-1">
              <button type="button" onClick={handleAdd} disabled={saving || !addForm.question.trim()} className="text-xs bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 disabled:opacity-50">
      ...
      {!loading && questions.length === 0 && (
        <div className="text-center py-12 text-sm text-gray-400">No questions in the repository yet.</div>
      )}
          ...
          if (isEditing) {
            return (
              <div key={q.id} className="border border-indigo-200 dark:border-indigo-900 rounded-lg p-3 bg-indigo-50/30 dark:bg-indigo-950/20">
                <div className="flex flex-col gap-2">
                  <textarea rows={2} value={editForm.question} onChange={(e) => setEditForm((f) => ({ ...f, question: e.target.value }))} className={INPUT_CLS + " resize-none text-sm"} />
                  <textarea rows={2} placeholder="Expected answer" value={editForm.expected_answer} onChange={(e) => setEditForm((f) => ({ ...f, expected_answer: e.target.value }))} className={INPUT_CLS + " resize-none text-sm"} />
                  <div className="grid grid-cols-3 gap-2">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-medium">{q.domain}</span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400">{q.category}</span>
                    {q.tags.map((t) => (
